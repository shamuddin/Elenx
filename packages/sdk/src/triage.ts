...
export enum TriageTier {
    ...
    TIER_3_SEMANTIC = "TIER_3_SEMANTIC"           // >500ms
}
...
export class TriageEngine {
     ...
     */
    private checkTier1(selector: string): TriageResult | null {
     ...
     */
    private checkTier2(intent: string, selector: string): TriageResult | null {
        const start = Date.now();
        const sel = selector.toLowerCase();
        ...
        if (isHighStakes) return null; // Escalate to Tier 3
    ...
    public async triage(intent: string, selector: string): Promise<TriageResult> {
        // Run Tier 1
