...
export class AdversarialScrubber {
    ...
    async scrub(page: Page): Promise<ScrubberReport> {
        ...
        const metadata = await page.evaluate(() => {
            ...
            function extractMetadata(root: any) {
                        ...
                        elements.forEach(function(el: any) {
                            ...
                            if (val === targetText) {
                                if (el.tagName === 'META') el.content = '[NEUTRALIZED_BY_SIF]';
                                    ...
                                    if (el.hasAttribute('aria-label')) el.setAttribute('aria-label', '[NEUTRALIZED_BY_SIF]');
                                    if (el.hasAttribute('alt')) el.setAttribute('alt', '[NEUTRALIZED_BY_SIF]');
                                    if (el.hasAttribute('title')) el.setAttribute('title', '[NEUTRALIZED_BY_SIF]');
                                    if (el.hasAttribute('aria-details')) el.setAttribute('aria-details', '[NEUTRALIZED_BY_SIF]');
                                    if (el.hasAttribute('aria-describedby')) el.setAttribute('aria-describedby', '[NEUTRALIZED_BY_SIF]');
                                }
                            }
                        });
                        const allElements = root.querySelectorAll('*');
                        allElements.forEach(function(el: any) { if (el.shadowRoot) findAndFix(el.shadowRoot); });
                    }
                    findAndFix(document);
                }, text);
            }
        ...
        await page.evaluate(function(resultsMap: any) {
