...
export class AgentMCP {
    ...
    private ensureSessionStarted(actionName: string) {
    ...
    public setRootIntent(intent: string) {
    ...
    public async verifyAction(action: string, callerId: string = "system", args: any = {}): Promise<{ isConsistent: boolean; message: string }> {
        ...
        if (!this.rootIntent) {
            ...
            const inference = await this.analyzer.inferIntent(action) as any;
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (actionName) {
                ...
                case "attest_capability":
                    const { serverId, manifest } = request.params.arguments as any;
                ...
                case "zk_verify":
                    {
                        const { proof, intent: zkIntent } = request.params.arguments as any;
                ...
                case "set_root_intent":
                    {
                        const r_intent = (request.params.arguments as any).intent;
                ...
                case "browse":
                    {
                        const { url, intent: browseIntent } = request.params.arguments as any;
