...
import { TriageEngine, TriageTier, SovereignTemplateEngine } from '@elenx/core';
...
export class SelfHealingKernel {
    ...
    async executeVerifiedTask(intent, url, selector, callerId = "system") {
        ...
        const result = await this.browser.execute(url, async (page) => {
            ...
            const liveContent = await page.evaluate((el) => el.textContent, element);
            const schemaMatch = goldenSchema.expectedElements.find((e) => e.selector === selector);
            ...
            if (isHighStakes || isPoisonedChain || isDirectlyUntrusted) {
                ...
                const view = this.sovereign.generateView(type, {
    ...
    async triggerHITL(action, callerId) {
        ...
        try {
            ...
            while (decision === 'pending') {
                ...
                const approval = statusRes.data.find((a) => a.id === approvalId);
    ...
    async heal(url, failedSelector, page, callerId = "system") {
        const goldenSchema = await this.scraper.getGoldenSchema(url);
        const alternative = goldenSchema.expectedElements.find((e) => e.name === 'price');
        if (alternative && alternative.selector !== failedSelector) {
            ...
            if (newElement) {
                return {
                    ...
                    data: await page.evaluate((el) => el.textContent, newElement)
                };
            }
        }
