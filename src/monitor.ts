import { MissionControlUI, TransactionalLedger, LedgerEntry } from '@elenx/core';
...
function updateUIFromLedger() {
    try {
        ...
        const chain: LedgerEntry[] = JSON.parse(content);
        ...
        for (const entry of newEntries) {
            ...
            if (data.type === 'mcp_intent_established' || data.type === 'mcp_intent_bootstrapped') {
                ...
                if (reason.includes('INTENT_LAUNDERING')) blockType = "PROVENANCE FAILURE";
                else if (reason.includes('UNAUTHORIZED_CALLER')) blockType = "IDENTITY FAILURE";
                else if (reason.includes('Infrastructure Bifurcation')) blockType = "BIFURCATION DETECTED";
...
async function startMonitor() {
    ...
    if (fs.existsSync(ledgerPath)) {
        try {
            const chain: LedgerEntry[] = JSON.parse(fs.readFileSync(ledgerPath, 'utf8'));
            ...
            if (lastIdentity) tui.updateTelemetry('NHI-ID', lastIdentity.data.id);
