...
export class SemanticAnalyzer {
    ...
    async analyzeDirective(text) {
        if (!text || text.length < 5)
            return { score: 0, reason: "Text too short", isAdversarial: false };
    ...
    async primaryVerification(intent, action) {
    ...
    async skepticalAudit(intent, action, primaryScore) {
     ...
     */
    async generateRecoveryGuide(intent, blockedAction, reason) {
